...
EDDM~\cite{baena2006early} 		& Early Drift Detection Method  	\\
...
Learn++.NSE~\cite{elwell2011incremental} & Learning in Non-Stationary Environments\\
...
STAGGERGeneratorF2BF  &  \textbf{100.0}  & 99.95 & 99.95 &  \textbf{100.0}  &  \textbf{100.0}  & 99.95 & 99.73 &  \textbf{100.0}  & 89.92 & 0.00 &  \textbf{100.0}  &  \textbf{100.0}  & -96.47 &   \\
...
\end{minipage}
\end{table}
Table~\ref{tab:DSResultsTrain} shows the training times, in seconds, for each algorithm. The Na\"ive Bayes classifier has the fastest run time and is ranked first. This is to be expected since it is the simplest algorithm out of the $13$. However, its accuracy and Cohen's kappa results were far from competitive. The DWM and HOT algorithms were ranked next, however, DWM's accuracy and Cohen's kappa rate were also statistically worse in comparison to the competing methods. Based on the previous performance metrics, the top algorithms were: OLLAWV, AdaHOT, EDDM, $k$-NNPAW, and HOT. In terms of run time, OLLAWV showed better scalability than AdaHOT, EDDM, and $k$-NNPAW, despite having to rebuild its classifier model with every received batch of data. These results indicate that OLLAWV shows a competitive performance with respect to training time against the top performing data stream methods.
