...
\end{tikzcd}\]
\end{minipage}
\caption{\small SVR Flow Diagram. Firstly, MT dataset is divided into $m$ ST datasets, $\mathcal{D}_1, \mathcal{D}_2, \ldots, \mathcal{D}_m$. Then $m$ models, $h_1, h_2, \ldots, h_m$, are independently trained for each ST dataset.}\label{diag:SVR}
\begin{algorithm}[H]
...
\resizebox{\textwidth}{!}{\begin{tabularx}{1.545\textwidth}{l@{\extracolsep{\fill}}ccccccccccccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
Dataset & OLLAWV & HOT & AdaHOT & NB & $k$-NNPAW & EDDM & VFDR & VFDR-NB & SAE2 & LearnNSE &DWM &DACC &OCBoost &\\
\noalign{\smallskip}\hline\noalign{\smallskip}
...
STAGGERGeneratorF1 &\textbf{100.000} &99.9429 &99.9429 &\textbf{100.000} &\textbf{100.000} &\textbf{100.000} &99.5649 &\textbf{100.000} &78.2768 &14.6120 &\textbf{100.000} &99.7795 &\textbf{100.000} &  \\
...
\end{minipage}
\end{table}
Table~\ref{tab:DSResultsTrain} shows the training times, in seconds, for each algorithm. The Na\"ive Bayes classifier has the fastest runtime and is ranked first. This is to be expected since it is the simplest algorithm out of the $13$. However, its accuracy and Cohen's kappa results were far from competitive. The DWM and HOT algorithms were ranked next, however, DWM's accuracy and Cohen's kappa rate were also statistically worse in comparison to the competing methods. Based on the previous performance metrics, the top algorithms were: OLLAWV, AdaHOT, EDDM, $k$-NNPAW, and HOT. In terms of runtime, OLLAWV showed better scalability than AdaHOT, EDDM, and $k$-NNPAW, despite having to rebuild its classifier model with every received batch of data. These results indicate that OLLAWV shows a competitive performance with respect to training time against the top performing data stream methods.
