...
Let $\proc$ be a data generating process which provides a sequence of tuples $\mathcal{S}_t = \set{\tup{\bm x_t,y_t}}^n$ that are sampled from an unknown probability distribution at some arbitrary time $t$, where $n$ is the number of tuples. The sub-script, time $t$, is used to explicitly assert the data's time-varying nature. $\bm x_t \in \reals^d$ represents a feature vector, and $y_t \in \ints$ is its discrete class label (in the non-binary classification case), both at time time $t$. The data may arrive to the algorithm in an online manner, i.e. one sample at a time: $\mathcal{S}_t = \set{\tup{\bm x_t,y_t}}$, or in a batch setting: $\mathcal{S}_t = \set{\tup{\bm x_t^1,y_t^1},\ldots,\tup{\bm x_t^n,y_t^n}}$. The single-sample setting occurs when $n = 1$.

\subsection{Data Stream Classification Methods}
As mentioned previously, there are two main approaches for solving evolving data stream problems: \textit{active} and \textit{passive}. This section will present the theory behind both approaches, their advantages and disadvantages, as well as current popular solvers. 

Before diving into the inner workings of each approach, there are some considerations for designing data stream algorithms that need to be introduced. Firstly, consider the case where $\proc$ generates a sequence of data that is possibly infinite in length. It is unrealistic to expect all the data acquired be available always~\cite{zang2014comparative}. It is more realistic to assume and accept that $\mathcal{S}_t$ is only available for learning at the time it is first presented to the algorithm, also known as \textit{one-pass} or \textit{incremental learning}~\cite{bifet2009new,polikar2001learn}. Secondly, most data stream algorithms expect that the predictions made by the classifier will be validated with $\mathcal{S}_t$'s labels, which arrive with the next training set, $\mathcal{S}_{t+1}$. This framework allows the algorithm to measure loss at each time step, known as the \textit{test-then-train} or \textit{prequential} approach~\cite{dawid1984present}. This implies that the evaluation of the classifier on the previous dataset is conducted prior to training with the next. 




\textit{Hoeffding Option Tree} (HT)~\cite{domingos2000mining},  (very-fast decision tree (VFDT))
This paper describes and evaluates VFDT, an anytime system that builds decision trees using constant memory and constant time per example. It uses Hoeffding bounds to guarantee that its output is asymptotically nearly identical to that of a conventional learner. 
...
\textit{Adaptive Hoeffding Option Tree} (AdaHT)~\cite{pfahringer2007new}, 
In this paper we explore Hoeffding Option Trees, a regular Hoeffding tree containing additional option nodes that allow several tests to be applied, leading to multiple Hoeffding trees as separate paths.  Adaptive decision option tree for streaming data with adaptive Naive Bayes classification at leaves. An Adaptive Hoeffding Option Tree is a Hoeffding Option Tree with the following improvement: each leaf stores an estimation of the current error. It uses an EWMA estimator with alpha = .2. The weight of each node in the voting process is proportional to the square of the inverse of the error.

\textit{Rule Classifier} (RC)~\cite{gama2011learning}, 
Very Fast Decision Rules algorithm. VFDR is designed for high-speed data streams. It is a single pass algorithm, that learns ordered and/or unordered rules. Using naive Bayes in VFDT like algorithms, is a well known technique since it was introduced in Gama et al. (2003). used in VFDR.
...
\textit{Single Classifier Drift} (SCD)~\cite{baena2006early},
In this work we define a method for detecting concept drift, even in
