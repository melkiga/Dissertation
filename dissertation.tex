...
\end{minipage}
\end{figure}
The optimization function given in~\ref{eq:softsvropt} can be solved more easily in its dual formulation and is key to extending the SVR to learn from non-linear functions~\cite{Schoelkopf2002}. 
...
\begin{table*}[t!]
\small \centering
\caption{Summary of Multiple-Target Learning Notation}
...
\State $\bm C = \sum_{i=1}^n |\bm{\Rho}_{ij}|, \forall j=1,\ldots,m$ \Comment{Sum row elements of the correlation coefficient matrix}
\State $\bm C = \mathtt{sort}\left(\bm C,\textbf{decreasing}\right)$ \Comment{Sort sums in decreasing order}
\State $h_{\bm C} = \mathtt{buildChainedModel} (\mathcal{D},\bm C)$ \Comment{build a chained model for max correlation chain $\bm C$} 
\end{algorithmic} 
\end{algorithm}
\end{figure}
...
According to the Wilcoxon test, SVRCC is shown to have significantly better performance over all algorithms with $p$-value $< 0.05$. The Nemenyi and Holm tests show that SVRCC performs better than $6$ out of the $9$ algorithms with $p$-value $\leq 5.6E^{-3}$ and $\leq 1.7E^{-2}$, respectively. The exact confidence for algorithm SVRCC against all others is $0.95$.
\begin{table}[H]
\centering \small
...
\captionof{table}{Wilcoxon, Nemenyi, and Holm tests for MSE}\label{tab:statmse}\vspace{-1.3em}
\scriptsize
\resizebox{0.95\textwidth}{!}{\begin{tabular}{lccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
SVRCC vs. & Wilcoxon $R^{+}$ & Wilcoxon $R^{-}$ & Wilcoxon $p$-value & Nemenyi $p$-value & Holm $p$-value \\ 
...
\captionof{table}{Wilcoxon, Nemenyi, and Holm tests for aRRMSE}\label{tab:statarrmse}\vspace{-1.3em}
\scriptsize
\resizebox{0.95\textwidth}{!}{\begin{tabular}{lccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
SVRCC vs. & Wilcoxon $R^{+}$ & Wilcoxon $R^{-}$ & Wilcoxon $p$-value & Nemenyi $p$-value & Holm $p$-value \\ 
...
\captionof{table}{Wilcoxon, Nemenyi, and Holm tests for Run Time}\label{tab:stattime}\vspace{-1.3em}
\scriptsize
\resizebox{0.95\textwidth}{!}{\begin{tabular}{lccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
SVRCC vs. & Wilcoxon $R^{+}$ & Wilcoxon $R^{-}$ & Wilcoxon $p$-value & Nemenyi $p$-value & Holm $p$-value \\ 
...
\node (l4) at (c4) [above right=.25cm and 0cm, align=right,scale=0.7] {\tiny Na\"{i}ve Bayes};
...
\captionof{figure}{Bonferroni-Dunn test for Accuracy}\label{fig:BonfDunnaccOther}\vspace{-3em}
\end{minipage}
\end{table}
...
Rank & 1.90 &\textbf{1.10} &\textbf{1.3333} & 1.6667  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\end{table}
...
\label{tab:dsalgs}
\resizebox{\textwidth}{!}{\begin{tabularx}{1.1\textwidth}{l@{\extracolsep{\fill}}l}
\hline\noalign{\smallskip}
Algorithm 	& Description 						\\
\noalign{\smallskip}\hline\noalign{\smallskip}
HOT~\cite{pfahringer2007new} 	& Hoeffding Option Tree 	\\
AdaHOT~\cite{pfahringer2007new} & Adaptive Hoeffding Option Tree			\\
NB~\cite{john1995estimating}	& Na\"{i}ve Bayes 					\\
$k$-NNPAW~\cite{bifet2013efficient}	& $k$-NN with Probabilistic Adaptive Windows \\
DDM~\cite{gama2004learning} 	& Drift Detection Method with HOT  	\\
VFDR~\cite{gama2011learning} 	& Very Fast Decision Rules 			\\
VFDR-NB~\cite{gama2011learning} & VFDR with Na\"{i}ve Bayes			\\
SAE2~\cite{gomes2014sae2}		& Social Adaptive Ensemble 2			\\
Learn.NSE~\cite{elwell2011incremental} & Learn++ for Non-Stationary Environments\\
DWM~\cite{kolter2007dynamic} 	& Dynamic Weighted Majority 	\\
DACC~\cite{jaber2013new}		& Dynamic Adaptation to Concept Changes 		\\
OCBoost~\cite{pelossof2009online} & Online Coordinate Boosting 		\\
\noalign{\smallskip}\hline
\end{tabularx}}
\end{table}
The experimental environment tests the difference in performance of OLLAWV against 12 contemporary algorithms, designed to handle stationary and non-stationary data streams. Some use explicit drift-detectors and others take a passive approach. The majority of the classifiers are without an explicit drift-detector because OLLAWV was implemented without one, ensuring a fair comparison. However, classifiers with a detector mechanisms were included for a more comprehensive understanding of how OLLAWV will compare. 
...
\newpage
Figure~\ref{fig:BonfDunnaccDS} shows the results for the statistical analysis on the accuracy results. The algorithms with rank higher than $5.48$ (OLLAWV's rank + the Bonferroni-Dunn critical value), i.e. to the right of the gray bar in Figure~\ref{fig:BonfDunnaccDS}, perform statistically worse than OLLAWV. The Bonferroni-Dunn test shows that there are no statistically significant differences in the accuracy results achieved by OLLAWV and AdaHOT, DDM, $k$-NNPAW, and HOT. This is expected firstly, since the AdaHOT and DDM methods utilize HOT as their base classifier and the HOT classifier performed competitively; and secondly, because of $k$-NNPAW's use of a probabilistic sliding window. 
\begin{table}[t!]
\caption{Accuracy (\%) for Data Stream Classifiers}
\scriptsize
\centering
\label{tab:DSResultsacc}
\resizebox{\textwidth}{!}{\begin{tabularx}{1.45\textwidth}{l@{\extracolsep{\fill}}ccccccccccccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
...
\begin{table}[b!]
\caption{Cohen's Kappa Rate (\%) for Data Stream Classifiers}
\scriptsize
\centering
...
Dataset & OLLAWV & HOT & AdaHOT & NB & $k$-NNPAW & DDM & VFDR & VFDR-NB & SAE2 & LearnNSE &DWM &DACC &OCBoost &\\
\noalign{\smallskip}\hline\noalign{\smallskip}
CovType  &  \textbf{84.30}  & 76.44 & 77.69 & 40.16 & 80.43 & 34.11 & 31.47 & 61.16 & 62.98 & 50.63 & 55.53 & 36.64 & 48.24 &   \\
Census  & 5.06 & 34.37 &  \textbf{37.39}  & 35.96 & 22.11 & 20.39 & 6.97 & 18.90 & 18.66 & 17.48 & 36.18 & 16.68 & 33.66 &   \\
Shuttle  & 96.17 & 94.91 & 95.85 & 75.04 &  \textbf{97.92}  & 95.75 & 60.82 & 89.53 & 75.32 & 81.56 & 75.10 & 74.98 & 36.69 &  \\ 
RBFNoDrift  &  \textbf{88.42}  & 85.88 & 85.92 & 43.98 & 87.49 & 84.96 & 55.07 & 63.41 & 78.32 & 40.57 & 40.84 & 30.02 & 84.17 &   \\
LEDNoDrift  & 70.92 & 70.94 & 70.93 &  \textbf{71.05}  & 62.03 & 70.71 & 34.62 & 70.84 & 64.00 & 64.27 & 67.94 & 42.52 & 8.27 &   \\
HyperplaneSlow  &  \textbf{80.18}  & 64.19 & 64.84 & 55.38 & 68.05 & 63.13 & 37.77 & 70.38 & 66.97 & 72.41 & 76.12 & 61.32 & 71.55 &   \\
HyperplaneFaster  &  \textbf{79.02}  & 65.43 & 70.68 & 54.46 & 68.54 & 68.65 & 57.27 & 70.36 & 67.63 & 73.00 & 73.52 & 62.31 & 75.59 &   \\
RBFGradualRecurring  & 97.88 & 92.82 & 92.58 & 44.28 &  \textbf{97.90}  & 91.27 & 46.66 & 81.51 & 84.61 & 63.73 & 66.54 & 49.00 & 31.46 & \\   
RBFBlips  &  \textbf{98.74}  & 94.14 & 94.06 & 46.97 & 98.57 & 93.13 & 55.08 & 84.27 & 85.20 & 69.57 & 72.85 & 56.99 & 32.73 &   \\
...
SEASuddenAN05  & 73.19 & 66.84 & 67.41 & 73.28 & 72.20 &  \textbf{74.92}  & 59.91 & 67.94 & 67.13 & 68.97 & 71.21 & 64.56 & 73.28 &   \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &\textbf{85.79} &83.37 &84.36 &69.36 &83.28 &82.59 &61.35 &77.61 &74.91 &60.48 &73.83 &63.12 &56.37 &  \\
Rank &\textbf{3.02} &5.54 &4.58 &7.75 &4.85 &4.58 &11.38 &6.60 &8.25 &8.92 &6.94 &11.15 &7.44 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\begin{minipage}{0.9\textwidth}
\centering
...
\node (l5) at (c5) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny DDM};
...
\fill[fill=gray,fill opacity=0.5] (3.0208,-0.1) rectangle (5.98,0.1);
...
\end{tikzpicture}}\vspace{-1em}
\captionsetup{width=\linewidth}
\captionof{figure}{Bonferroni-Dunn test for Cohen's Kappa rate}\label{fig:BonfDunnkappaDS}
\end{minipage}
\end{table}
\subsection{Cohen's Kappa Rate}
...
\subsection{Training \& Testing Time}
\begin{table}[b!]
\caption{Training Time (seconds) for Data Stream Classifiers}
\scriptsize
\centering
\label{tab:DSResultsTrain}
\resizebox{\textwidth}{!}{\begin{tabularx}{1.45\textwidth}{l@{\extracolsep{\fill}}cccccccccccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
Dataset & OLLAWV & HOT & AdaHOT & NB & $k$-NNPAW & DDM & VFDR & VFDR-NB & SAE2 & LearnNSE &DWM &DACC &OCBoost \\
\noalign{\smallskip}\hline\noalign{\smallskip}
...
\end{table}
\begin{table}[b!]
\caption{Testing Time (seconds) for Data Stream Classifiers}
\scriptsize
\centering
\label{tab:DSResultsTest}
\resizebox{\textwidth}{!}{\begin{tabularx}{1.45\textwidth}{l@{\extracolsep{\fill}}cccccccccccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
Dataset & OLLAWV & HOT & AdaHOT & NB & $k$-NNPAW & DDM & VFDR & VFDR-NB & SAE2 & LearnNSE &DWM &DACC &OCBoost \\
\noalign{\smallskip}\hline\noalign{\smallskip}
CovType &0.0391 &0.0256 &0.0321 &0.0089 &1.3744 &0.0041 &\textbf{0.0010} &0.0304 &0.0493 &0.1896 &0.0358 &0.0170 &0.0325   \\
Census &0.0223 &0.0082 &0.0095 &0.0033 &2.2826 &0.0014 &\textbf{0.0012} &0.0082 &0.0048 &0.1052 &0.0125 &0.0076 &0.0099   \\
...
HyperplaneFaster02 &0.0272 &0.0088 &0.0118 &0.0016 &0.5671 &\textbf{0.0012} &0.0019 &0.0048 &0.0174 &0.0790 &0.0081 &0.0037 &0.0085   \\
...
\coordinate (c0) at (10.375,0);
\coordinate (c1) at (4.75,0);
\coordinate (c2) at (5.625,0);
\coordinate (c3) at (3.3125,0);
\coordinate (c4) at (12.875,0);
\coordinate (c5) at (1.6458,0);
\coordinate (c6) at (2.8333,0);
\coordinate (c7) at (7.0417,0);
\coordinate (c8) at (8.9583,0);
\coordinate (c9) at (11.9583,0);
\coordinate (c10) at (7.0417,0);
\coordinate (c11) at (5.7917,0);
\coordinate (c12) at (8.7917,0);

\node (l0) at (c0) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny \textbf{OLLAWV}};
\node (l1) at (c1) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny HOT};
\node (l2) at (c2) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny AdaHOT};
\node (l3) at (c3) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny NB};
\node (l4) at (c4) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny kNNPAW};
\node (l5) at (c5) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny DDM};
\node (l6) at (c6) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny VFDR};
\node (l7) at (c7) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny VFDR-NB};
\node (l8) at (c8) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny SAE2};
\node (l9) at (c9) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny LearnNSE};
\node (l10) at (c10) [above right=.1cm and 0cm, align=right,scale=0.7] {\tiny DWM};
\node (l11) at (c11) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny DACC};
\node (l12) at (c12) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny OCBoost};

\fill[fill=gray,fill opacity=0.5] (1.6458,-0.1) rectangle (4.7,0.1);
...
\end{minipage}
\end{table}
Tables~\ref{tab:DSResultsTrain} and~\ref{tab:DSResultsTest} show the training and testing times in seconds for each algorithm. The Na\"ive Bayes classifier has the fastest training time and is ranked first, while exhibiting fast testing times. This is to be expected since it is the simplest algorithm out of the $13$ compared. However, its accuracy and Cohen's kappa rate results were far from competitive. The DWM and HOT algorithms were ranked next in terms of training time, however, DWM's accuracy and Cohen's kappa rate were also statistically significantly worse in comparison to the competing methods. Fast execution times are insignificant without complementing performance results, as in the case with the Na\"ive Bayes and VFDR. 
