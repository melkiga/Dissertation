...
\resizebox{\textwidth}{!}{\begin{tabularx}{1.42\textwidth}{l@{\extracolsep{\fill}}ccccccccccccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
Dataset & OLLAWV & HOT & AdaHOT & NB & $k$-NNPAW & EDDM & VFDR & VFDR-NB & SAE2 & LearnNSE &DWM &DACC &OCBoost &\\
\noalign{\smallskip}\hline\noalign{\smallskip}
...
The overall results, obtained by the batch data stream experiments, highlighted the better performance OLLAWV and popular algorithms capable of adapting to changes in the data's distribution over time. This serves as future inspiration for developing a decremental unlearning procedure for the OLLAWV algorithm.

Another possible change stems from one of OLLAWV's unique attributes. OLLAWV operates by selecting and updating a worst violating sample per iteration. However, for online streams, there is no way of knowing which samples are/will be worst-violators, unless they are gathered and compared to one another. Due to this, OLLAWV (in its present condition) may not be able to be adapted to the online stream environment. OLLA-L2, on the other hand, would fit nicely in the pure online setting, since it proceeds sample by sample, with no sample order required. The main issue with OLLA-L2 is its lack of meaningful stopping criteria, possibly resulting in a dense model, which would deteriorate its runtime performance. Due to OLLAWV's competitive performance against the contemporary stream methods in this experimental study, as well as OLLA-L2's similar efficiency, it is worth investigating combining OLLAWV with the positive attributes that OLLA-L2 possesses, as well as implementing a decremental learning mechanism, to further improve this family of online algorithms under the stationary and non-stationary data stream setting.
