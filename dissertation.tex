...
Time Step & $t$ \\
Input Space & $\bm{X} \in \mathbb{R}^{n \times d}$ \\
Labels & $\bm{Y} \in \ints^n$ \\
Sample $i$ & $\bm{x}_i = (x_{i1}, \ldots, x_{id}),\, \forall i \in \set{1,\ldots,n}$ \\
Sample Label $i$ & $y_i \in \set{-1,1},\, \forall i \in \set{1,\ldots,n}$ \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Data Generating Process & $\proc = \set{(\bm x_1,y_1), (\bm x_2,y_2), \ldots, (\bm x_t,y_t), \ldots}$ \\
Set of Samples Provided to the Algorithm & $\mathcal{S}_t = \set{\tup{\bm x_t^1,y_t^1},\ldots,\tup{\bm x_t^n,y_t^n}}$ \\
\noalign{\smallskip}\hline
\end{tabularx}
\end{table}
...
Ensemble classifiers are popular approaches for learning in streamed environments and provide some advantages: they tend to have more flexibility in easily incorporating newly presented data into a model by adding new members to the ensemble; they also obtain a natural way of ``\textit{forgetting}" irrelevant knowledge by removing members from the ensemble~\cite{ditzler2015learning}. The two most common ensemble methods are bagging and boosting~\cite{oza2001onlinebb}. One example of an approach that uses sample and model weighting/selection to obtain the best accuracy over different sections of a drifting stream is the \textit{Dynamic Weighted Majority} (DWM)~\cite{kolter2007dynamic} algorithm. The main idea is to choose the best classifier which performs well for an unseen sample or portion of the training stream. Another ensemble method is the \textit{Social Adaptive Ensemble 2} (SAE2)~\cite{gomes2014sae2}, an improved version of SAE~\cite{gomes2013sae} classifier. SAE2 maintains a classifier ensemble arranged in a network, where connections are created between classifiers that have similar predictions, as done in SAE. However, SAE2 implements a more scalable adaptation method: the classifier's connections are updated before being added to the ensemble. The main disadvantages of such ensemble approaches stem from their high computational and spacial complexity, as well as their low inter-classifier interpretability.

One of the most well known online, ensemble approach is the \textit{Dynamic Weighted Majority} (DWM) algorithm~\cite{kolter2007dynamic}. Each classifier in the ensemble has a weight that is scaled down by some constant, $0 \leq \beta < 1$, if it makes an incorrect prediction, thus allowing the ensemble to emphasize classifiers that are most accuracy at a point in time. Furthermore, the ability to add/remove classifiers is feasible in order to accelerate reacting to a possible concept drift. Another relatively new online ensemble is the \textit{Anticipative Dynmic Adaptation to Concept Changes} (ADACC)~\cite{jaber2013new} algorithm, which recognizes concepts from incoming samples and optimizes control over the incremental classifiers.

Another ensemble approach, belonging to the Learn++ family of algorithms~\cite{polikar2001learn}, is the method called \textit{Learn++ for Non-Stationary Environments} (Learn++.NSE)~\cite{elwell2011incremental}. Unlike the previously mentioned ensemble methods, Learn++.NSE operates in batch mode, and has the ability to learn from environments that exhibit a constant, variable, or cyclical drifts. It incrementally trains a new classifier for each batch received and combines the classifiers using a novel dynamic weighted majority voting method. 

\textit{$k$-Nearest neighbor} ($k$-NN) methods comprise another class of algorithms that have been adapted to handle non-stationary streams of data, however, they use a \textit{change-detection} mechanism. Unlike the VFDTs and Na\"{i}ve Bayes classifiers, $k$-NN cannot learn from a stream without discarding data. These methods maintain a dynamic sample, or \textit{sliding window}, of the data stream. Finding this representative sample of data is referred to as \textit{reservoir sampling}~\cite{vitter1985random}, a change-detection technique used by many streaming algorithms~\cite{ditzler2015learning}. Basic implementations of $k$-NN maintain a sliding window of fixed sizes because storing all samples in a stream is not feasible due to memory and processing constraints. The class label is then assigned by the majority label present among the $k$-nearest neighbors of the unknown test sample. It is worth noting that CVFDT's main adaptation from the VFDT algorithm also utilizes a sliding window. 

An example of such windowing methods is the \textit{Probabilistic Adaptive Window} (PAW) algorithm~\cite{bifet2013efficient}, used with $k$-NN. Rather than storing all samples, or a limited sized window, PAW keeps a dynamically sized window with a logarithmic number of samples, storing those with higher probability than the rest. Another example of windowing techniques is that of the \textit{ADaptive WINdowing} (ADWIN) algorithm~\cite{bifet2007learning}, which increases the size of the window when no change is apparent, and shrinks it when there is a drift. The main disadvantage with these change detection techniques is their difficulty to set a threshold for when a change occurs; too low of a threshold value would cause many false positives, and too large a threshold would cause many false negatives. 
...
Support vector machine (SVM) methods have infrequently been used for data stream classification due to challenges that stem from incrementally updating the SVM model as the number of samples increases. Some methods, such as SVMLight~\cite{Joachims1999} and SVMPerf~\cite{joachims2006training}, have been proposed for speeding up SVM classification. However, the biggest issue with these algorithms is that they were not designed for streaming environments. Generalizing SVMs to the data stream setting is particularly critical because of their generalization capabilities, scalability, and robustness against outliers, as mentioned in Chapter~\ref{chap:background}. The methods mentioned employ a quadratic programming formulation, with equal number of constraints as the number of data, making the problem computationally expensive for large amounts of data. In the kernelized case, the size of the kernel scales with the number of samples squared, an impractical solution in the case of data streams.

It is worth noting that some of the earliest work in streamed SVM learning~\cite{klinkenberg2000detecting,syed1999handling} precedes the earliest work in stream decision tree learning. In fact, the contributions made by Klinkenberg and Joachims in~\cite{klinkenberg2000detecting}, are one of the earliest window-based methods for detecting concept drift. Other significant contributions on incremental, or online, support vector machines are discussed in~\cite{bordes2005fast,cauwenberghs2001incremental,domeniconi2001incremental,fung2002incremental,shalev2011pegasos,ralaivola2001incremental,ruping2001incremental,shilton2005incremental}.

One of the main concepts in the methods listed above is that SVM classification is non-linear programming problem that can be solved using Karush-Kuhn-Tucker (KKT) conditions. This implies that, as long as the addition and removal of test samples is accounted for on these conditions, the approach will most likely succeed~\cite{aggarwal}. In other words, the KKT optimality conditions must be maintained when adding or removing samples. Developing approaches such as these are more likely to be more computationally efficient than retraining an SVM from the beginning and would definitely be useful in the streaming scenario, especially when a drift occurs. The most popular and widely used SVM method is the algorithm presented by Cauwenberghs and Poggio~\cite{cauwenberghs2001incremental} because of its notable decremental learning procedure, which provides geometrical insights into the relationship between the generalization performance and the shape of the data~\cite{aggarwal}.


\section{OnLine Learning Algorithms for Batched Data Streams}
Two online, iterative learning algorithms, OLLA-L2~\cite{kecman2016fast} and OLLAWV~\cite{melki2018ollawv}, are implemented and tested within the context of a batched data stream environment. 

OLLA-L2 stems from the same core algorithm as OLLAWV, OLLA~\cite{kecman2016fast}, a generic stochastic learning algorithm tailored for solving various risk functions, regularized or not. The implementation used in this Chapter minimizes the primal L1-SVM cost function, as done in OLLAWV, and is listed in Algorithm~\ref{alg:List2L1SVM}. At every iteration, the algorithm proceeds cyclically over the data, updating the model if the sample, $i$, chosen is a violator, i.e. has an error ($y_i o_i \leq 1$). If the sample has no error ($y_i o_i > 1$), the model remains the same. In the context of batched streams, the algorithm cycles through the current batch and for a pure online scenario, the algorithm proceeds and updates as long as there are samples provided by the stream. The quantity of iterations can be controlled by the parameter $e$ representing the number of epochs. As mentioned previously, early stopping acts as a regularization technique~\cite{collobert2004links}, and this can be controlled through setting the number of epochs to be less than the number of samples.


When $e \leq 1$, the parameter $\Rho$ is always 0. 
...
\Ensure  $\bm \alpha$, $b$, $\bm S$
\State $\bm \alpha \leftarrow \bm 0, b \leftarrow 0, \bm S \leftarrow \bm 0$
\State $\bm o \leftarrow \bm 0,\, i \leftarrow 0$
\For{$t = 1, \ldots, n*e$}
...
The update parameters $\Lambda \in \reals$ (lambda) and $\Rho \in \reals$ (rho) describe scalars which are used for updating the output vector $\bm o \in \reals^n$, the weight vector in kernel feature space $\bm \alpha \in \reals^n$, and the bias term $b \in \reals$, if it is used. The input parameter $\beta \in \set{0,1}$ describes whether the bias term will be used ($\beta = 1$) or not ($\beta = 0$). The update parameters for $\Lambda$ and $\Rho$ for the L1-SVM with regularizer $||\bm w||^2$ are given by:
\begin{table}[H]
\centering \small
\begin{tabular}{cc}
...
\textbf{Discuss drift detection mechanism a bit.}

\begin{table}[th!]
\centering
\caption{Comparison of OLLAWV vs. DSL2SVM}
\scriptsize
\label{tab:dslvsolla}
...
\noalign{\smallskip}\hline\noalign{\smallskip}
RBFNoDrift &93.0663 &\textbf{94.2117} &\textbf{0.0218} &0.0329   \\
HyperplaneSlow &87.4016 &\textbf{90.0913} &\textbf{0.0261} &0.0353   \\
HyperplaneFaster &87.4033 &\textbf{89.5092} &\textbf{0.0256} &0.0263   \\
STAGGERGeneratorF1 &\textbf{100.0000} &\textbf{100.0000} &0.0034 &\textbf{0.0021}  \\
HyperplaneFaster0.2 &87.4047 &\textbf{89.4911} &\textbf{0.0257} &0.0268   \\
...
STAGGERGeneratorF1BF &\textbf{100.0000} &\textbf{100.0000} &0.0035 &\textbf{0.0021}   \\
STAGGERGeneratorF2BF &\textbf{100.0000} &\textbf{100.0000} &0.0039 &\textbf{0.0022}  \\
...
HyperplaneFasterCN0 &87.4033 &\textbf{89.5092} &\textbf{0.0257} &0.0263   \\
HyperplaneFasterACN0 &87.4033 &\textbf{89.5092} &\textbf{0.0257} &0.0263   \\
SEASuddenAN0 &84.0088 &\textbf{87.7956} &0.0494 &\textbf{0.0208}   \\
SEASuddenAN05 &83.6874 &\textbf{87.5283} &0.0494 &\textbf{0.0284}  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &91.3093 &\textbf{93.4148} & \textbf{0.0207} & 0.0210 \\
Rank &1.9118 &\textbf{1.0882} &\textbf{1.2941} & 1.7059  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\end{table}
...
\State Initialize $\mathtt{learner.setModelContext(InstancesHeader\,\,header)}$
\While{stream has more instances}
\State Generate new batch of instances, $\mathcal{S}_t$
\If{!$\mathtt{firstChunk}$}
\For{each test instance}
\State $\mathtt{double[]\,\,pred = learner.getVotesForInstance(Instance\,\,instance)}$ 
\EndFor
\Else 
\State $\mathtt{firstChunk =\,FALSE}$
\EndIf
\State $\mathtt{learner.trainOnInstances(Instances\,\,chunk)}$
\EndWhile
\end{algorithmic} 
\end{algorithm}
The experimental environment was designed to test the difference in performance of the proposed method against 12 contemporary algorithms belonging to two categories: those with and without concept drift detectors. The majority of the classifiers used in the experimental environment are without a concept drift detector because OLLAWV was implemented without one, ensuring a fair comparison. However, classifiers with a drift detector were included for a more comprehensive understanding of how OLLAWV will compare when a concept drift arises. 

The algorithms without a drift detector are as follows: \textit{Hoeffding Option Tree} (HT)~\cite{hulten2001mining}, \textit{Adaptive Hoeffding Option Tree} (AdaHT)~\cite{bifet2009adaptive}, \textit{Na\"ive Bayes}, \textit{Rule Classifier} (RC) and \textit{Rule Classifier Na\"ive Bayes} (RCNB)~\cite{gama2011learning}, \textit{Social Adaptive Ensemble Classifier} (SAE2)~\cite{gomes2014sae2}, and \textit{Online Coordinate Boosting} (OCBoost)~\cite{pelossof2009online}. Those with a drift detector are: \textit{$k$-Nearest Neighbors with Probabilistic Adaptive Window} ($k$-NNPAW), \textit{Single Classifier Drift} (SCD)~\cite{baena2006early} using the \textit{Hoeffding Tree} as a base, \textit{Learning in Non-Stationary Environments} (LearnNSE)~\cite{elwell2011incremental}, \textit{Dynamic Weighted Majority} (DWM)~\cite{kolter2007dynamic}, \textit{Dynmic Adaptation to Concept Changes} (DACC)~\cite{jaber2013new}. These algorithms were chosen because they have shown considerable performance in learning in a stream environment, while also being readily available for use and reproducing their results through MOA, Massive Online Analysis\footnote{https://moa.cms.waikato.ac.nz/}~\cite{moacite}. Experiments were run on a server with 2 Intel Xeon CPU E5-2690v4 with 28 cores (56 threads), 128 GB of memory, and CentOS 7.4. OLLAWV was implemented in Java within the MOA framework. 

The experiments were performed using the evaluation method named \textit{Interleaved Test-Then-Train}, also known as \textit{Prequential}~\cite{moacite}, listed in Algorithm~\ref{alg:evalChunk}, which is used to describe the accuracy (or any other evaluation metric) of algorithms over time. In this procedure, each sample can be used to test the model before it is used during training, and using this, the evaluation metric can be incrementally updated. Using this order, the algorithms are always being tested on unseen samples. This scheme ensures that no hold-out test set is needed, thus utilizing all available data. It also provides a smooth plot of the evaluation metric over time, where each sample becomes increasingly less significant to the overall average. 
...
\end{tabularx}
\end{table}
Table~\ref{tab:dsDataset} presents a summary of the static datasets and the stream generators used throughout the experiments, where the number of attributes, classes, and samples are shown. Both the static datasets and the generators provided 1000-sample sized batches to the algorithms. The static datasets were obtained from the UCI Machine Learning repository\footnote{http://archive.ics.uci.edu/ml/index.php}~\cite{Lichman:2013} and were simulated as streams using a stream generator from the MOA framework. The stream generators were also used within the MOA~\cite{moacite} framework. The following are descriptions of the types of generators used as base generators throughout the experimental study. Variations of the generators were also used to simulate slow and abrupt concept drifts.
\begin{itemize}
\iitem \textbf{Random RBF (Radial Basis Function) Generator}: This generator produces samples that form normally distributed hyperspheres surrounding randomly selected centroids, with varying densities. This generator also has the option to simulate data evolution, i.e. concept drift. 
...
\iitem \textbf{Waveform Generator}: This generator constructs three different types of waves which are combinations of two or three base waveforms. The goal is to determine the final wave type.
...
\end{align}
\end{subequations}
The training times of each algorithm are also given in Table~\ref{tab:DSResultsTrain} to analyze the scalability and speed of each of the algorithms. These metrics were all recorded using the Test-Then-Train procedure. 

In order to analyze the performances of the multiple models, non-parametric statistical tests are used to validate the experimental results obtained. The Iman-Davenport non-parametric test is run to investigate whether significant differences exist among the performance of the algorithms by ranking them over the datasets used, using the Friedman test. The algorithm ranks for each metric in Equations~\eqref{eq:metrics} are presented in the last row of the results tables, and the lowest (best) rank value is typeset in bold. Table~\ref{tab:DSResultsmeta} contains the ranks and meta-rank of all methods, which helps determine and visualize the best performing algorithms across all datasets and metrics. 

After the Iman-Davenport test indicates significant differences, the Bonferroni-Dunn post-hoc test~\cite{Dunn1961} is then used to find where they occur between algorithms by assuming the classifiers' performances are different by at least some critical value. Below each result table, a figure highlighting the critical distance (in gray), from the best ranking algorithm to the rest, is shown. The algorithms to the right of the critical distance bar perform statistically significantly worse than the control algorithm, OLLAWV. Figures \ref{fig:BonfDunnaccDS}, \ref{fig:BonfDunnkappaDS}, and \ref{fig:BonfDunnTrainDS} show the results of the Bonferroni-Dunn post-hoc procedure over the metrics in~\eqref{eq:metricsds}, as well as the meta-rank results in Table~\ref{tab:DSResultsmeta}.
...
\noalign{\smallskip}\hline\noalign{\smallskip}
Dataset & OLLAWV & HT & AdaHT & Naive Bayes & $k$-NNPAW &SCD & RC &RCNB & SAE2 & LearnNSE &DWM &DACC &OCBoost &\\
\noalign{\smallskip}\hline\noalign{\smallskip}
CovType &\textbf{90.2822} &85.3405 &86.2222 &60.0371 &87.8893 &59.5562 &60.3240 &75.5822 &76.2069 &69.9669 &71.9559 &61.7002 &71.2860 &  \\
census &93.7574 &94.6946 &\textbf{94.7352} &87.0188 &93.6456 &91.8530 &93.6477 &84.0601 &90.1319 &84.1443 &91.4037 &90.3708 &93.4695 &  \\
shuttle &98.6536 &98.1750 &98.5214 &90.0161 &\textbf{99.2643} &98.4893 &88.3982 &96.0571 &90.2429 &93.7893 &89.9125 &92.0268 &74.2107 &  \\
RBFNoDrift &\textbf{94.2117} &92.9410 &92.9598 &71.9924 &93.7469 &92.4787 &77.5293 &81.7067 &89.1612 &70.2820 &70.4251 &65.0084 &92.0836 &  \\
LEDNoDrift &73.8281 &73.8508 &73.8349 &\textbf{73.9421} &65.8277 &73.6379 &41.1618 &73.7521 &67.5965 &67.8404 &71.1464 &48.2683 &17.4397 &  \\
HyperplaneSlow &\textbf{90.0913} &82.0965 &82.4205 &77.6904 &84.0264 &81.5657 &68.8829 &85.1884 &82.6670 &86.2044 &88.0601 &80.6617 &85.7758 &  \\
HyperplaneFaster &\textbf{89.5092} &82.7166 &85.3419 &77.2308 &84.2722 &84.3261 &78.6329 &85.1810 &83.0129 &86.4991 &86.7611 &81.1543 &87.7953 &  \\
RBFGradualRecurring &98.4136 &94.6248 &94.4420 &58.3269 &\textbf{98.4261} &93.4687 &60.0797 &86.1603 &88.4771 &72.8730 &74.9565 &61.9133 &49.6157 &  \\
RBFBlips &\textbf{99.0651} &95.6659 &95.6028 &60.8269 &98.9389 &94.9151 &66.8955 &88.3527 &89.0376 &77.5314 &79.9797 &68.2728 &47.4572 &  \\
WaveformGenerator &83.9375 &82.9866 &\textbf{84.1385} &80.4105 &80.1250 &83.6086 &63.8796 &75.8405 &80.1837 &80.1908 &78.3940 &73.5897 &55.0493 &  \\
STAGGERGeneratorF1 &\textbf{100.0000} &99.9887 &99.9887 &\textbf{100.0000} &\textbf{100.0000} &\textbf{100.0000} &99.9140 &\textbf{100.0000} &95.0427 &89.8206 &\textbf{100.0000} &99.9564 &\textbf{100.0000} &  \\
HyperplaneFaster0.2 &\textbf{89.4911} &82.7744 &85.3603 &77.2520 &84.2736 &87.6381 &78.8852 &85.1126 &83.0403 &86.5075 &86.7563 &81.2289 &87.5864 &  \\
RBFGradualRecurringv2 &\textbf{97.1781} &93.2890 &93.0019 &57.4707 &95.7334 &93.1871 &57.9634 &80.7124 &84.4510 &62.4141 &63.7931 &49.4211 &48.8806 &  \\
MixedGeneratorBT &97.9969 &99.1064 &\textbf{99.3219} &91.9333 &97.6650 &99.1067 &83.1220 &93.2806 &93.1578 &90.9064 &91.1943 &89.1559 &98.9794 &  \\
MixedGeneratorBF &98.0284 &99.1773 &\textbf{99.3600} &92.0423 &97.5911 &99.2032 &89.9633 &94.3045 &93.4079 &90.7614 &91.4588 &88.6093 &98.9444 &  \\
RandomRBFGeneratorC4A25 &\textbf{99.1244} &97.4330 &97.1381 &81.5943 &98.6904 &96.8900 &72.3260 &89.0275 &90.6129 &78.2290 &79.6715 &63.0206 &52.1561 &  \\
RandomRBFGeneratorC4A50 &\textbf{99.7429} &99.1592 &99.1373 &91.9012 &99.1654 &98.9934 &80.6333 &95.6272 &92.0019 &86.0263 &90.4494 &73.7765 &50.6618 &  \\
SineGeneratorF1BF &97.7909 &\textbf{99.7491} &99.7276 &93.5510 &95.5395 &99.6634 &94.8262 &95.9013 &94.5098 &92.5533 &93.3025 &92.1967 &99.5085 &  \\
SineGeneratorF2BF &97.7909 &\textbf{99.7464} &99.7264 &93.5510 &95.4139 &99.6620 &95.2625 &96.0973 &94.5689 &92.5533 &93.3383 &92.1967 &99.4790 &  \\
STAGGERGeneratorF1BF &\textbf{100.0000} &99.9887 &99.9887 &\textbf{100.0000} &\textbf{100.0000} &\textbf{100.0000} &99.9140 &\textbf{100.0000} &95.0427 &89.8206 &\textbf{100.0000} &99.9564 &\textbf{100.0000} &  \\
STAGGERGeneratorF2BF &\textbf{100.0000} &99.9774 &99.9774 &\textbf{100.0000} &\textbf{100.0000} &99.9774 &99.8683 &\textbf{100.0000} &95.0189 &44.4047 &\textbf{100.0000} &\textbf{100.0000} &0.6121 &  \\
RBFGradualRecurringCN0 &98.4136 &94.6248 &94.4420 &58.3269 &\textbf{98.4261} &93.4687 &60.0797 &86.1603 &88.4771 &72.8730 &74.9565 &61.9133 &49.6157 &  \\
RBFGradualRecurringACN0 &98.4136 &94.6248 &94.4420 &49.6157 &\textbf{98.4261} &93.4687 &60.0797 &85.9703 &88.4771 &72.8730 &74.9565 &61.9133 &49.6157 &  \\
HyperplaneFasterAN0 &\textbf{89.5092} &82.7166 &85.3419 &87.7953 &84.2722 &84.3261 &78.6329 &85.1810 &83.0129 &86.4991 &86.7611 &81.1543 &87.7953 &  \\
HyperplaneFasterAN5 &\textbf{89.2898} &82.6891 &85.2441 &87.3759 &84.1852 &88.7366 &79.1352 &84.8853 &82.9208 &86.4050 &86.6744 &81.1184 &87.3759 &  \\
HyperplaneFasterCN0 &\textbf{89.5092} &82.7166 &85.3419 &87.7953 &84.2722 &84.3261 &78.6329 &85.1810 &83.0129 &86.4991 &86.7611 &81.1543 &87.7953 &  \\
HyperplaneFasterACN0 &\textbf{89.5092} &82.7166 &85.3419 &87.7953 &84.2722 &84.3261 &78.6329 &85.1810 &83.0129 &86.4991 &86.7611 &81.1543 &87.7953 &  \\
SEASuddenAN0 &87.7956 &84.9212 &85.1746 &88.2325 &87.2164 &\textbf{88.9726} &81.5637 &85.1719 &85.1104 &85.7668 &86.9326 &83.7264 &88.2325 &  \\
SEASuddenAN05 &87.5283 &84.5692 &84.8189 &87.5277 &86.9567 &\textbf{88.2862} &81.5456 &85.1097 &84.5655 &85.6288 &86.5414 &83.5301 &87.5277 &  \\
CovTypeAN0 &\textbf{90.2822} &85.3405 &86.2222 &71.2860 &87.8893 &59.5562 &60.3240 &75.5822 &76.2069 &69.9669 &71.9559 &61.7002 &71.2860 &  \\
CovTypeCN0 &\textbf{90.2822} &85.3405 &86.2222 &71.2860 &87.8893 &59.5562 &60.3240 &75.5822 &76.2069 &69.9669 &71.9559 &61.7002 &71.2860 &  \\
CovTypeACN0 &\textbf{90.2822} &85.3405 &86.2222 &71.2860 &87.8893 &59.5562 &60.3240 &75.5822 &76.2069 &69.9669 &71.9559 &61.7002 &71.2860 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &\textbf{93.4284} &90.5963 &91.2426 &80.1597 &91.3103 &87.9000 &75.9808 &86.6104 &86.0870 &80.1957 &83.7241 &76.6640 &73.4563 &  \\
Rank &\textbf{2.2031} &5.6562 &4.5000 &7.9531 &4.7031 &5.7500 &11.2500 &6.7344 &8.1250 &8.7500 &6.9844 &10.9531 &7.4375 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\begin{minipage}{0.9\textwidth}
\centering
...
\coordinate (c0) at (2.2031,0);
\coordinate (c1) at (5.6562,0);
\coordinate (c2) at (4.5,0);
\coordinate (c3) at (7.9531,0);
\coordinate (c4) at (4.7031,0);
\coordinate (c5) at (5.75,0);
\coordinate (c6) at (11.25,0);
\coordinate (c7) at (6.7344,0);
\coordinate (c8) at (8.125,0);
\coordinate (c9) at (8.75,0);
\coordinate (c10) at (6.9844,0);
\coordinate (c11) at (10.9531,0);
\coordinate (c12) at (7.4375,0);

\node (l0) at (c0) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny \textbf{OLLAWV}};
\node (l1) at (c1) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny HT};
\node (l2) at (c2) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny AdaHT};
\node (l3) at (c3) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny NaiveBayes};
\node (l4) at (c4) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny $k$-NNPAW};
\node (l5) at (c5) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny SCD};
\node (l6) at (c6) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny RC};
\node (l7) at (c7) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny RCNB};
...
\fill[fill=gray,fill opacity=0.5] (2.2031,-0.1) rectangle (4.99,0.1);
...
RandomRBFGeneratoC4A25 &\textbf{98.8172} &96.5331 &96.1343 &75.2231 &98.2308 &95.7997 &62.3923 &85.1851 &87.3386 &70.6112 &72.5571 &49.6768 &36.2515 &  \\
...
SineGeneratorF2BF &95.5500 &\textbf{99.4896} &99.4493 &86.9837 &90.7381 &99.3195 &90.4504 &92.1487 &89.0741 &84.9693 &86.5495 &84.2484 &98.9512 &  \\
STAGGERGeneratoF1BF &\textbf{100.0000} &99.9429 &99.9429 &\textbf{100.0000} &\textbf{100.0000} &\textbf{100.0000} &99.5649 &\textbf{100.0000} &78.2768 &14.6120 &\textbf{100.0000} &99.7795 &\textbf{100.0000} &  \\
...
RBFGradualRecurringCN0 &97.8807 &92.8205 &92.5769 &44.2773 &\textbf{97.8977} &91.2740 &46.6574 &81.5090 &84.6106 &63.7269 &66.5421 &49.0047 &31.4648 &  \\
RBFGradualRecurringACN0 &97.8807 &92.8205 &92.5769 &31.4648 &\textbf{97.8977} &91.2740 &46.6574 &81.2726 &84.6106 &63.7269 &66.5421 &49.0047 &31.4648 &  \\
HyperplaneFasterAN0 &\textbf{79.0184} &65.4333 &70.6839 &75.5906 &68.5439 &68.6522 &57.2656 &70.3619 &67.6281 &72.9982 &73.5220 &62.3085 &75.5906 &  \\
...
HyperplaneFasterCN0 &\textbf{79.0184} &65.4333 &70.6839 &75.5906 &68.5439 &68.6522 &57.2656 &70.3619 &67.6281 &72.9982 &73.5220 &62.3085 &75.5906 &  \\
HyperplaneFasterACN0 &\textbf{79.0184} &65.4333 &70.6839 &75.5906 &68.5439 &68.6522 &57.2656 &70.3619 &67.6281 &72.9982 &73.5220 &62.3085 &75.5906 &  \\
...
CovTypeAN0 &\textbf{84.2997} &76.4406 &77.6946 &48.2357 &80.4264 &34.1059 &31.4704 &61.1629 &62.9842 &50.6291 &55.5296 &36.6365 &48.2357 &  \\
CovTypeCN0 &\textbf{84.2997} &76.4406 &77.6946 &48.2357 &80.4264 &34.1059 &31.4704 &61.1629 &62.9842 &50.6291 &55.5296 &36.6365 &48.2357 &  \\
CovTypeACN0 &\textbf{84.2997} &76.4406 &77.6946 &48.2357 &80.4264 &34.1059 &31.4704 &61.1629 &62.9842 &50.6291 &55.5296 &36.6365 &48.2357 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &\textbf{85.7720} &81.6278 &82.9678 &65.9918 &82.5460 &77.2796 &57.2445 &75.6249 &73.7176 &60.9312 &71.6284 &59.6790 &55.8496 &  \\
Rank &\textbf{2.5781} &5.7500 &4.5312 &7.7031 &4.7344 &5.6250 &11.6562 &6.6094 &7.9688 &8.4688 &6.7344 &11.1406 &7.5000 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\begin{minipage}{0.9\textwidth}
\centering
...
\coordinate (c0) at (2.5781,0);
\coordinate (c1) at (5.75,0);
\coordinate (c2) at (4.5312,0);
\coordinate (c3) at (7.7031,0);
\coordinate (c4) at (4.7344,0);
\coordinate (c5) at (5.625,0);
\coordinate (c6) at (11.6562,0);
\coordinate (c7) at (6.6094,0);
\coordinate (c8) at (7.9688,0);
\coordinate (c9) at (8.4688,0);
\coordinate (c10) at (6.7344,0);
\coordinate (c11) at (11.1406,0);
\coordinate (c12) at (7.5,0);

\node (l0) at (c0) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny \textbf{OLLAWV}};
\node (l1) at (c1) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny HT};
\node (l2) at (c2) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny AdaHT};
\node (l3) at (c3) [above right=.25cm and 0cm, align=right,scale=0.7] {\tiny NaiveBayes};
\node (l4) at (c4) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny $k$-NNPAW};
\node (l5) at (c5) [above left=.2cm and 0cm, align=right,scale=0.7] {\tiny SCD};
\node (l6) at (c6) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny RC};
\node (l7) at (c7) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny RCNB};
...
\fill[fill=gray,fill opacity=0.5] (2.5781,-0.1) rectangle (5.367,0.1);
...
census &0.1684 &0.1040 &0.1162 &\textbf{0.0007} &0.0386 &0.0345 &0.0735 &0.0458 &0.0363 &0.9368 &0.0156 &0.0175 &0.0543 &  \\
...
RandomRBFGeneratorC4A25 &0.0157 &0.0333 &0.0337 &\textbf{0.0007} &0.0350 &0.0909 &2.8802 &2.5559 &0.1917 &27.8456 &0.0752 &0.1432 &0.2974 &  \\
SineGeneratorF1BF &0.0122 &0.0056 &0.0056 &\textbf{0.0001} &0.0334 &0.0073 &1.9301 &2.9169 &0.0127 &1.3768 &0.0034 &0.0071 &0.0235 &  \\
SineGeneratorF2BF &0.0121 &0.0051 &0.0053 &\textbf{0.0001} &0.0314 &0.0077 &5.0598 &5.9477 &0.0117 &1.3996 &0.0027 &0.0074 &0.0236 &  \\
...
RBFGradualRecurringCN0 &0.0457 &0.0304 &0.0304 &\textbf{0.0003} &0.0352 &0.0319 &0.8493 &0.8675 &0.0933 &10.9096 &0.0323 &0.0595 &0.1689 &  \\
RBFGradualRecurringACN0 &0.0458 &0.0395 &0.0398 &0.1359 &\textbf{0.0313} &0.0457 &1.0166 &0.8629 &0.1209 &12.1337 &0.0356 &0.0592 &0.1646 &  \\
HyperplaneFasterAN0 &0.0263 &0.0206 &0.0213 &0.0446 &0.0263 &0.0841 &0.1776 &0.1865 &0.0281 &2.3635 &\textbf{0.0080} &0.0154 &0.0715 &  \\
HyperplaneFasterAN5 &0.0264 &0.0200 &0.0200 &0.0440 &0.0269 &0.0115 &0.2298 &0.2417 &0.0312 &2.5014 &\textbf{0.0080} &0.0147 &0.0740 &  \\
HyperplaneFasterCN0 &0.0263 &0.0323 &0.0209 &0.0406 &3.1129 &0.0968 &0.2061 &175.2788 &0.0309 &2.5008 &\textbf{0.0072} &0.0658 &0.0697 &  \\
HyperplaneFasterACN0 &0.0263 &0.0398 &0.0400 &0.0503 &0.0313 &0.1149 &0.2727 &0.2797 &0.0469 &3.4281 &\textbf{0.0097} &0.0175 &0.0697 &  \\
...
CovTypeAN0 &0.0444 &0.0873 &0.0897 &0.1579 &\textbf{0.0315} &0.0585 &0.0383 &0.0375 &0.1208 &6.5487 &0.0425 &0.0554 &0.1953 &  \\
CovTypeCN0 &0.0450 &0.0456 &0.0447 &0.0951 &0.0270 &0.0276 &\textbf{0.0227} &0.0229 &0.0696 &3.9599 &0.0261 &0.0338 &0.1610 &  \\
CovTypeACN0 &0.0450 &0.0451 &0.0455 &0.1001 &0.0267 &0.0280 &\textbf{0.0236} &0.0240 &0.0689 &3.9489 &0.0254 &0.0341 &0.1613 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &0.0341 &0.0296 &0.0334 &0.0227 &0.1307 &0.0483 &0.7003 &6.2028 &0.0579 &5.6078 &\textbf{0.0190} &0.0308 &0.0829 &  \\
Rank &6.5938 &4.9062 &5.6875 &3.4375 &6.9688 &6.2500 &9.2188 &9.2188 &8.2188 &12.6875 &\textbf{2.7188} &5.5000 &9.5938 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\begin{minipage}{0.9\textwidth}
\centering
...
\coordinate (c0) at (6.5938,0);
\coordinate (c1) at (4.9062,0);
\coordinate (c2) at (5.6875,0);
\coordinate (c3) at (3.4375,0);
\coordinate (c4) at (6.9688,0);
\coordinate (c5) at (6.25,0);
\coordinate (c6) at (9.2188,0);
\coordinate (c7) at (9.2188,0);
\coordinate (c8) at (8.2188,0);
\coordinate (c9) at (12.6875,0);
\coordinate (c10) at (2.7188,0);
\coordinate (c11) at (5.5,0);
\coordinate (c12) at (9.5938,0);

\node (l0) at (c0) [above right=.3cm and 0cm, align=right,scale=0.7] {\tiny \textbf{OLLAWV}};
\node (l1) at (c1) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny HT};
\node (l2) at (c2) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny AdaHT};
\node (l3) at (c3) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny NaiveBayes};
\node (l4) at (c4) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny $k$-NNPAW};
\node (l5) at (c5) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny SCD};
\node (l6) at (c6) [above right=.25cm and 0cm, align=right,scale=0.7] {\tiny RC};
\node (l7) at (c7) [above right=.1cm and 0cm, align=right,scale=0.7] {\tiny RCNB};
\node (l8) at (c8) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny SAE2};
...
\fill[fill=gray,fill opacity=0.5] (2.7188,-0.1) rectangle (5.789,0.1);
...
\subsection{Overall Comparison}
\begin{table}[t!]
\caption{Meta-Ranks Comparison for Data Stream Classifiers}
\scriptsize
\centering
\label{tab:DSResultsmeta}
\resizebox{\textwidth}{!}{\begin{tabularx}{1.28\textwidth}{l@{\extracolsep{\fill}}cccccccccccccc}
\noalign{\smallskip}\hline\noalign{\smallskip}
Metric & OLLAWV & HT & AdaHT & NaiveBayes & $k$-NNPAW & SCD & RC & RCNB & SAE2 & LearnNSE &DWM &DACC &OCBoost \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Accuracy &\textbf{2.2031} &5.6562 &4.5000 &7.9531 &4.7031 &5.7500 &11.2500 &6.7344 &8.1250 &8.7500 &6.9844 &10.9531 &7.4375 &  \\
Cohen's Kappa Rate &\textbf{2.5781} &5.7500 &4.5312 &7.7031 &4.7344 &5.6250 &11.6562 &6.6094 &7.9688 &8.4688 &6.7344 &11.1406 &7.5000 &  \\
Training Time &6.5938 &4.9062 &5.6875 &3.4375 &6.9688 &6.2500 &9.2188 &9.2188 &8.2188 &12.6875 &\textbf{2.7188} &5.5000 &9.5938 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &\textbf{3.7917} &5.4375 &4.9062 &6.3646 &5.4688 &5.8750 &10.7083 &7.5208 &8.1042 &9.9688 &5.4792 &9.1979 &8.1771 &  \\
Rank &\textbf{3.0000} &4.0000 &\textbf{3.0000} &6.6667 &4.6667 &5.0000 &12.1667 &7.5000 &9.6667 &11.6667 &5.0000 &9.3333 &9.3333 &  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\begin{minipage}{0.9\textwidth}
\centering
...
\coordinate (c3) at (6.6667,0);
\coordinate (c4) at (4.6667,0);
\coordinate (c5) at (5,0);
\coordinate (c6) at (12.1667,0);
\coordinate (c7) at (7.5,0);
\coordinate (c8) at (9.6667,0);
\coordinate (c9) at (11.6667,0);
\coordinate (c10) at (5,0);
\coordinate (c11) at (9.3333,0);
\coordinate (c12) at (9.3333,0);

\node (l0) at (c0) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny \textbf{OLLAWV}};
\node (l1) at (c1) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny HT};
\node (l2) at (c2) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny AdaHT};
\node (l3) at (c3) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny NaiveBayes};
\node (l4) at (c4) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny kNNPAW};
\node (l5) at (c5) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny SCD};
\node (l6) at (c6) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny RC};
\node (l7) at (c7) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny RCNB};
\node (l8) at (c8) [below right=.1cm and 0cm, align=right,scale=0.7] {\tiny SAE2};
\node (l9) at (c9) [above right=.25cm and 0cm, align=right,scale=0.7] {\tiny LearnNSE};
\node (l10) at (c10) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny DWM};
\node (l11) at (c11) [above right=.1cm and 0cm, align=right,scale=0.7] {\tiny DACC};
\node (l12) at (c12) [above left=.25cm and 0cm, align=right,scale=0.7] {\tiny OCBoost};

\fill[fill=gray,fill opacity=0.5] (3,-0.1) rectangle (5.789,0.1);
...
\captionsetup{width=\linewidth}
\captionof{figure}{Bonferroni-Dunn test for Meta-Ranks}\label{fig:BonfDunnmetaDS}
\end{minipage}
\end{table}
