...
OLLA-L2~\cite{kecman2016fast} and OLLAWV~\cite{melki2018ollawv}, two online, iterative learning algorithms are implemented and investigated within the context of a batched, stationary and non-stationary, data stream environment. OLLA-L2 stems from the same core algorithm as OLLAWV, simply named \textit{OnLine Learning Algorithm} (OLLA)~\cite{kecman2016fast}. It is a generic stochastic learning algorithm, in the primal domain, tailored and designed for various non-linear (i.e. kernel) classifiers. In this implementation of OLLA-L2, similar to the implementation of OLALWV described in Chapter~\ref{chap:ollawv}, the online learning algorithm is derived and used for solving the primal L1-SVM cost function, and is listed in Algorithm~\ref{alg:List2L1SVM}. The online nature of the core OLLA algorithm and its competitive performance in terms of speed and accuracy against popular and widely used SVM solvers merited investigation in the data stream setting. \textit{The implementation and experiments conducted with OLLA-L2 will first be discussed, before moving onto the final contribution OLLAWV.}

At every iteration, OLLA-L2 proceeds cyclically over the data, updating the model if the chosen sample, $i$, is a violator, i.e. has an error ($y_i o_i \leq 1$). If the sample has no error ($y_i o_i > 1$), the model remains the same and no update is performed. The update parameters $\Lambda \in \reals$ (lambda) and $\Rho \in \reals$ (rho), as seen in Algorithm~\ref{alg:List2L1SVM}, describe scalars which are used for updating the output vector $\bm o \in \reals^n$, the weight vector in kernel feature space $\bm \alpha \in \reals^n$, and the bias term $b \in \reals$, if it is used. The input parameter $\beta \in \set{0,1}$ describes whether the bias term will be used ($\beta = 1$) or not ($\beta = 0$). The update parameters for $\Lambda$ and $\Rho$ for the L1-SVM with regularizer $||\bm w||^2$ are given by:
\begin{table}[H]
\centering \small
\begin{tabular}{cc}
...
In the context of batched streams, the algorithm would cycle through the samples in each arriving batch. For a pure online scenario, the algorithm would proceed with its updates, as long as samples are provided by the stream. In the batch setting, the quantity of iterations can be controlled by the parameter $e$, which represents the number of epochs, or cycles, over the data. As mentioned in the previous chapter, early stopping acts as a regularization technique~\cite{collobert2004links}, and this can be controlled through setting the number of epochs to be less than the number of samples. It is worth noting that, if the number if epochs is set to be less than the number of samples, or in the pure online data stream learning setting where samples arrive one at a time with no cycling involved, the parameter $\Rho$ is always equal to 0. $\Rho > 0$ only if the algorithm cycles over already updated samples since $\bm \alpha$ is initialized to 0. 

The advantage of using OLLA-L2's update procedure is most apparent in lines 8 and 15 in Algorithm~\ref{alg:List2L1SVM}. The model's $\bm \alpha$ parameter update is simply the sum of two scalars values: the previous value of $\bm \alpha$ and $\Lambda$. Then, to determine whether an update is even needed, the next sample's output value, $o_i$, is needed. It is obtained by the kernel function which involves a scalar product of the sample against all currently saved support vectors (stored in $\bm S$). 
...
Although OLLA-L2 has many advantages, it also has its complexities, some of which are shared by other stochastic solvers. One example would be how/what to set the algorithm's maximum number of iterations, i.e. the model's optimal point to stop training. \textit{Stopping the training too early might cause missed opportunity for finding all the support vectors. Stopping too late might cause the algorithm to diverge from its optimum point.} This complexity inspired the work in Chapter~\ref{chap:ollawv} on OLLAWV. The novelty and advantage of OLLAWV resides in the way it iterates and updates the model. At each step, the algorithm chooses and updates the most violating sample, i.e. \textit{worst-violator}. A worst-violator is defined as the sample that produces the largest error with respect to the current decision function. Rather than shuffling the data and cycling through each sample, as done in OLLA-L2, OLLAWV selects (without replacement) the most incorrectly classified sample and updates the model accordingly. This procedure is essentially finding the model's support vectors and implicitly defining a stopping criterion. If there are no more violating samples, that algorithm terminates. \textit{These characteristics are especially useful in the context of learning from a streamed environment. continue about why, then segway to the comparison on L2 and OLLAWV...}
\begin{table}[b!]
\centering
...
\end{tabularx}}
\end{table}
Preliminary experiments were conducted to test the capabilities of both algorithm approaches. The results in Table~\ref{tab:dslvsolla} show the better performance of OLLAWV in terms of accuracy. Both algorithms scale similarly in terms of runtime. The main bottleneck in terms of runtime for OLLAWV is an $n \times 1$ sized vector (which decreases) calculation per iteration. However, the time is regained by the algorithm early stopping. 

The better accuracy results motivated OLLAWV's implementation and further experimentation in the context of batch data streams. 
