...
\State $\bm o \leftarrow \bm 0$
\State $i \leftarrow 0, yo \leftarrow Y_i \bm o_i,\, max\_iter \leftarrow e*n$ 
\While{$t < 1, \ldots, max\_iter$}
\State $\eta \leftarrow 2/\sqrt{t}$
\If{$yo \leq 1$}
\State \textit{Calculate $\Lambda$ and $\Rho$}
\State $\bm S \leftarrow [\bm S i]$
...
\State $yo \leftarrow Y_i\, o_i$
\EndWhile
\end{algorithmic} 
\end{algorithm}

The update parameters $\Lambda \in \reals$ (lambda) and $\Rho \in \reals$ (rho) describe scalars which are used for updating the output vector $\bm o \in \reals^n$, the weight vector in kernel feature space $\bm \alpha \in \reals^n$, and the bias term $b \in \reals$, if it is used. The input parameter $\beta \in \set{0,1}$ describes whether the bias term will be used ($\beta = 1$) or not ($\beta = 0$). 

The update parameters for $\Lambda$ and $\Rho$ for the L1-SVM with regularizer $||\bm w||^2$ are given by:
\begin{table}[H]
\centering \small
\begin{tabular}{cc}
...
\noalign{\smallskip}\hline\noalign{\smallskip}
RBFNoDrift &93.0663 &\textbf{94.2117} &\textbf{0.0073} &0.0329 \\
HyperplaneSlow &87.4016 &\textbf{90.0913} &\textbf{0.0087} &0.0353 \\
HyperplaneFaster &87.4033 &\textbf{89.5092} &\textbf{0.0085} &0.0263  \\
STAGGERGeneratorF1 &\textbf{100.000} &\textbf{100.000} &\textbf{0.0011} &0.0021  \\
HyperplaneFaster0.2 &87.4047 &\textbf{89.4911} &\textbf{0.0086} &0.0268  \\
MixedGeneratorBT &92.4990 &\textbf{97.9969} &\textbf{0.0036} &0.0205  \\
MixedGeneratorBF &92.5537 &\textbf{98.0284} &\textbf{0.0036} &0.0299  \\
SineGeneratorF1BF &97.3659 &\textbf{97.7909} &\textbf{0.0030} &0.0122  \\
SineGeneratorF2BF &97.3659 &\textbf{97.7909} &\textbf{0.0030} &0.0121  \\
STAGGERGeneratorF1BF &\textbf{100.000} &\textbf{100.000} &\textbf{0.0012} &0.0021  \\
STAGGERGeneratorF2BF &\textbf{100.000} &\textbf{100.000} &\textbf{0.0013} &0.0022  \\
HyperplaneFasterAN0 &87.4033 &\textbf{89.5092} &\textbf{0.0085} &0.0263  \\
HyperplaneFasterAN5 &87.2912 &\textbf{89.2898} &\textbf{0.0086} &0.0264   \\
HyperplaneFasterCN0 &87.4033 &\textbf{89.5092} &\textbf{0.0086} &0.0263   \\
HyperplaneFasterACN0 &87.4033 &\textbf{89.5092} & \textbf{0.0086} &0.0263   \\
SEASuddenAN0 &84.0088 &\textbf{87.7956} & \textbf{0.0165} &0.0208  \\
SEASuddenAN05 &83.6874 &\textbf{87.5283} & \textbf{0.0165} &0.0284  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
Average &91.3093 &\textbf{93.4148} & \textbf{0.0069} &0.0210  \\
Rank &1.9118 &\textbf{1.0882} &1.0000 &2.0000  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabularx}}
\end{table}
...
4. Function classifiers: include related methods based on neural network and sup- port vector machines. Support vector machines (SVM) offer better flexibility, while neural networks are often straightforward to use.
...
5. Function Generator was introduced by Agrawal et al. in [4]. The gener- ated stream should model loan application profiles. The generator produces a stream, each instance of which consists of nine attributes: six numeric and three categorical.
...
9. The newest release of MOA also contains a multi-label data stream genera- tor [26], not described in [9].
...
RandomTreeGenerator 	   	 & 	 1,000,000	 & 	5	              & 	2 \\
MixedGenerator 					 & 	 1,000,000	 & 	4	              & 	2 \\%Learning with drift detection.
AssetNegotiationGenerator 	 & 	 1,000,000	 & 	18	           & 	7 \\
\noalign{\smallskip}\hline
\end{tabularx}
\end{table}
...
\node (l0) at (c0) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny OLLAWV};
\node (l1) at (c1) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny HT};
\node (l2) at (c2) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny AdaHT};
\node (l3) at (c3) [above left=.1cm and 0cm, align=right,scale=0.7] {\tiny NaiveBayes};
\node (l4) at (c4) [below left=.1cm and 0cm, align=right,scale=0.7] {\tiny $k$NNPAW};
