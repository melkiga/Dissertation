...
\subsection{Stochastic Gradient Descent for the L1-SVM Problem}
SVMs were typically treated and solved as a constrained quadratic optimization in dual space. Their early development was hindered because of the quaratic dependence, on the number of samples, of the memory required to efficiently solve them. This lead to the idea of optimizing over subsets of the data, also known as decomposition methods~\cite{boser1992training,Joachims1999,keerthi2001improvements,Platt1998}. Although these methods improved convergence rates, in practice, their superlinear (sometimes cubic) dependence on the number of samples still was an issue in terms of excessive runtimes when learning from massive datasets. Linear SVMs, taking advantage of linear kernels, were shown to outperform decomposition SVMs, motivating research for solving the SVM problem in the primal~\cite{Chapelle2007,panagiotakopoulos2013stochastic}. These algorithms are mostly based on the perceptron~\cite{freund1999large,rosenblatt1958perceptron} - the simplest online, i.e. stochastic, learning algorithm for binary linear classification in the primal space. It cycles repeatedly, one sample at a time, updaing each samples current state in the weight vector, until an appropriate condition is satisfied. By updating cyclically, one sample at a time, these types of algorithms are able to process large amounts of data at a much faster speeds with low memory resources, consequently making them suitable for handling large datasets.

\textbf{More text about why the primal is good.}

\textit{One approach, one similar to that of the approach of the large-margin perceptron, aims to solve the regularized soft margin loss through stochastic gradient descent. Notable representations of these algorithms can be found in Chapter~\ref{chap:background}. SGD allows a perceptron-like update, which involves the "shrinking" of the weight vector. This update is the only modification the algorithm performs when a new sample is presented, and only occurs if a some loss is incurred. This update is performed until some user intervention, i.e. due to a lack of meaningful stopping criteria and without user intervention, the algorithm keeps running forever. Thus, these types of algorithms that use SGD are fundementally different than ones such as mistake-driven large-margin perceptron-like approaches. }
...
\begin{equation}
\bm{w}^{(t+1)} = \bm{w}^{(t)} - \eta\bigtriangledown f(\bm{w}^{(t)})
...
\caption{Right-hand side of Equation~\ref{eqn:convexsubgr} is the tangent of $f$ at $\bm{w}$, where for convex functions, the tangent lower bounds $f$.}
\label{fig:stronglyconvex}
\end{figure}

The Gradient Descent algorithm requires that the function being minimized is differentiable. Some loss functions are not differentiable, and in these cases, the subgradient of $f(\bm{w})$ at $\bm{w}^{(t)}$ can be taken instead of the gradient. A vector $\bm{v}$ that satisfies Equation~\ref{eqn:convexsubgr} is called a \textit{subgradient} of $f$ at $\bm{w}$ and the set of subgradients is called the \textit{differential set} denoted as $\delta f(\bm{w})$. For every $\bm{w} \in S$, where $S$ is an open convex set, there exists a $\bm{v}$ such that
\begin{equation}
f(\bm{u}) \geq f(\bm{w}) + \langle\bm{u}-\bm{w},\bm{v}\rangle, \, \forall\bm{u}\in S.
...
\caption{A plot of the gradient descent algorithm (left) and the stochastic gradient descent algorithm (right). The function being minimized is $1.25(x+6)^2+(y-8)^2$. In the figure representing stochastic gradient descent, the solid line shows the averaged value of $\bm{w}$. }
\label{fig:SGDvsGD}
\end{figure}

For the stochastic gradient descent algorithm, the update direction does not need to be based exactly on the gradient. It is instead allowed to be a random vector, and at each iteration, its \textit{expected value} is required to be a subgradient of the function a the current vector. An example of the difference between the SGD and the GD procedure for minimization of convex function is shown in Fig.~\ref{fig:SGDvsGD}.
...
\begin{equation}
w^{(t+1)} = -\frac{1}{\lambda\,t}\sum_{i=1}^t v_i,
